# Fraud Detection Architecture Image Gallery

## Final architecture exports

Các ảnh dưới đây là bộ tám sơ đồ cuối, sinh từ
[`../AN_FINAL_ARCHITECTURE_DIAGRAMS.md`](../AN_FINAL_ARCHITECTURE_DIAGRAMS.md).

1. [Overall system architecture](final/overall-system-architecture.png)
2. [Layered data architecture](final/layered-data-architecture.png)
3. [Detailed data processing pipeline](final/detailed-data-processing-pipeline.png)
4. [Leakage-controlled transformation flow](final/leakage-controlled-transformation-flow.png)
5. [Data, feature and evaluation contracts](final/data-feature-evaluation-contracts.png)
6. [Model training lifecycle](final/model-training-lifecycle.png)
7. [Temporal development windows](final/temporal-development-windows.png)
8. [Promotion state machine](final/promotion-state-machine.png)

Các gallery phía dưới là export lịch sử, được giữ lại để traceability.

Các ảnh dưới đây được export riêng từ Mermaid source trong
[`../ARCHITECTURE_DIAGRAMS.md`](../ARCHITECTURE_DIAGRAMS.md). Nhấn vào tên ảnh
để mở file PNG gốc độ phân giải cao.

## 1. System overview

[Mở `01-system-overview.png`](png/01-system-overview.png)

![System overview](png/01-system-overview.png)

## 2. Data Pipeline

[Mở `02-data-pipeline.png`](png/02-data-pipeline.png)

![Data Pipeline](png/02-data-pipeline.png)

## 3. Temporal windows

[Mở `03-temporal-windows.png`](png/03-temporal-windows.png)

![Temporal windows](png/03-temporal-windows.png)

## 4. Data contract

[Mở `04-data-contract.png`](png/04-data-contract.png)

![Data contract](png/04-data-contract.png)

## 5. Model training lifecycle

[Mở `05-model-training-lifecycle.png`](png/05-model-training-lifecycle.png)

![Model training lifecycle](png/05-model-training-lifecycle.png)

## 6. Training runtime sequence

[Mở `06-training-sequence.png`](png/06-training-sequence.png)

![Training sequence](png/06-training-sequence.png)

## 7. Spark deployment

[Mở `07-spark-deployment.png`](png/07-spark-deployment.png)

![Spark deployment](png/07-spark-deployment.png)

## 8. Promotion state machine

[Mở `08-promotion-state-machine.png`](png/08-promotion-state-machine.png)

![Promotion state machine](png/08-promotion-state-machine.png)

## 9. Best Model V2 serving

[Mở `09-best-model-v2-serving.png`](png/09-best-model-v2-serving.png)

![Best Model V2 serving architecture](png/09-best-model-v2-serving.png)
