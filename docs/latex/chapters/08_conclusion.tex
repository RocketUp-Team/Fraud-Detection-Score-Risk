...
V2 lacks operating-point metrics, it should be described as the offline champion
rather than proof of production benefit. The later \texttt{0.0.2} XGBoost
candidate provides calibrated thresholds and confusion counts but has a lower
holdout PR-AUC of 0.4318 and has not been promoted.
...
\begin{itemize}
  \item local-only experiment history with no registry/promotion/checksum
  lineage;
...
\begin{enumerate}
  ...
  \item reproduce \texttt{0.0.2} operating evidence on independent windows and
  compare it with V2;
